        ...
        if (typeof config2.submit === "function") {
          ...
          if (!query2.callback) {
            result = new this._Promise((resolve2, reject) => {
              query2.callback = (err, res) => err ? reject(err) : resolve2(res);
            }).catch((err) => {
  ...
  "node_modules/pg/lib/native/client.js"(exports, module) {
    ...
    Client2.prototype._pulsePipelinedQueryQueue = function() {
      ...
      while (this._queryQueue.length > 0) {
        ...
        const pipelineEntry = { text: query2.text, name: query2.name };
        if (values) {
...
for (const file of [".env.local", ".env"]) {
  ...
  if (existsSync(path)) (0, import_dotenv.config)({ path, override: false, quiet: true });
}
...
var SERIALIZATION_FAILURE = "40001";
types2.setTypeParser(20, (value) => {
  const n = Number(value);
...
async function query(text, params = []) {
  ...
  try {
    await client2.query("SET database = interlock");
    return await client2.query(text, params);
  } finally {
    client2.release();
  }
}
async function serializableTx(fn, { maxAttempts = 5, label = "tx" } = {}) {
  const client2 = await getPool().connect();
  try {
    await client2.query("SET database = interlock");
...
var MODELS = {
  embeddings: process.env.BEDROCK_MODEL_EMBEDDINGS || "amazon.titan-embed-text-v2:0",
  bulk: process.env.BEDROCK_MODEL_BULK || "us.anthropic.claude-haiku-4-5-20251001-v1:0",
  adjudicator: process.env.BEDROCK_MODEL_ADJUDICATOR || "us.anthropic.claude-sonnet-4-6",
  escalate: process.env.BEDROCK_MODEL_ESCALATE || "us.anthropic.claude-opus-4-5-20251101-v1:0"
};
var PRICING = {
  "us.anthropic.claude-haiku-4-5-20251001-v1:0": { in: 1, out: 5 },
  "us.anthropic.claude-sonnet-4-6": { in: 3, out: 15 },
  "us.anthropic.claude-sonnet-4-5-20250929-v1:0": { in: 3, out: 15 },
  "us.anthropic.claude-opus-4-5-20251101-v1:0": { in: 5, out: 25 },
  "amazon.titan-embed-text-v2:0": { in: 0.02, out: 0 }
};
var WH_PER_1K_TOKENS = Number(process.env.ENERGY_WH_PER_1K_TOKENS ?? 0.3);
var G_CO2E_PER_KWH = Number(process.env.GRID_G_CO2E_PER_KWH ?? 379);
var client = new BedrockRuntimeClient({ region: REGION });
var Usage = class {
  ...
  get energy() {
    const wh = this.tokensTotal / 1e3 * WH_PER_1K_TOKENS;
    const whWasted = this.tokensWasted / 1e3 * WH_PER_1K_TOKENS;
    return {
      wh,
      kwh: wh / 1e3,
      gCO2e: wh / 1e3 * G_CO2E_PER_KWH,
      whWasted,
      gCO2eWasted: whWasted / 1e3 * G_CO2E_PER_KWH,
      assumption: `${WH_PER_1K_TOKENS} Wh/1k tokens \xB7 ${G_CO2E_PER_KWH} gCO2e/kWh (estimate)`
    };
  }
  toJSON() {
    return {
      ...
      wastedPct: this.tokensTotal ? +(this.tokensWasted / this.tokensTotal * 100).toFixed(1) : 0,
...
`;
async function findThreatened(commitId, { k = SEMANTIC_K, threshold = SEMANTIC_THRESHOLD } = {}) {
  const { rows } = await query(THREATENED_SQL, [commitId, k, threshold]);
  return rows;
}
async function diffForIntent(intentId) {
  ...
  for (const row of ir) {
    ...
    const changed = past ? past.version !== row.current_version : row.observed_version !== row.current_version;
    if (changed) {
      changes.push({
...
ORDER BY ps.seq
`;
async function dependentSteps(intentId, commitId) {
...
async function adjudicate({ commitId, intentId, usage, tier = null }) {
  ...
  if (dependent.length === 0) {
    ...
    return {
      verdict: "irrelevant",
      rationale: "No plan step descends from the changed resource. Settled by the provenance graph, with no inference required.",
      affectedSteps: [],
...
${diff.changes.length === 0 ? "No tracked resource changed value." : diff.changes.map(
    (c) => `- ${c.kind}/${c.key}: v${c.thenVersion} <-\u2019 v${c.nowVersion}
...
(Requested ${found.name}; unavailable: ${found.error})`;
  ...
  const data = res.data ?? {
    ...
    rationale: "Adjudicator returned unparseable output; defaulting to invalidating so the conflict is not silently ignored.",
    affected_steps: steps.map((s) => s.seq)
  };
  const verdict = ["irrelevant", "invalidating", "fatal"].includes(data.verdict) ? data.verdict : "invalidating";
  const affected = Array.isArray(data.affected_steps) ? data.affected_steps.filter((n) => Number.isInteger(n)) : [];
  return {
    verdict,
...
async function resolve({ commitId, intentId, adjudication, detectedBy, distance }) {
    ...
    async (client2) => {
      ...
      if (adjudication.verdict === "irrelevant") {
        ...
        for (const seq of adjudication.affectedSteps) {
          const { rows: st } = await client2.query(
